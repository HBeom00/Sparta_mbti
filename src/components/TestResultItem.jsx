...
const TestResultItem = ({ el, userInfo, updateResult, deleteResult }) => {
  return (
    <div style={{ border: "1px solid black", marginTop: "10px" }}>
      ...
      <div>
        <p>{el.result}</p>
        <p>{mbtiDescriptions[el.result]}</p>
        <p>{el.visibility}</p>
      </div>
      <div>
        {el.userId === userInfo ? (
          <>
            <button
              ...
              onClick={() => {
                updateResult({ id: el.id, visibility: el.visibility });
                {
                  el.visibility
                    ? alert("비공개 처리로 변경됩니다.")
                    : alert("공개 처리로 변경됩니다.");
